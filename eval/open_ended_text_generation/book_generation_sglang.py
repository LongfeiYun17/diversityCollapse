...
def process_output(output):
    template = (
        f"Return the book name in the following text:\n\n{output}\n\n"
        "The answer should be formalized as \"X\" without explanation. If there is no book name, return \"None\"."
    )
...
if __name__ == "__main__":
    ...
    batched_prompts = get_prompts("book_generation", dataset, args)
    ...
    outputs_path = Path('data') / f"outputs_book_generation_eval_{mode}_{args.prompt_mode}.json"
    ...
    outputs_path = Path('data') / f"outputs_with_labels_book_generation_eval_{mode}_{args.prompt_mode}.json"
    with open(outputs_path, "w") as f:
        json.dump(results, f, indent=4)
    with open('eval/open_ended_text_generation/results/book_generation_results.txt', 'a') as f:

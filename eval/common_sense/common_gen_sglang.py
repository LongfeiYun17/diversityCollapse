...
if __name__ == "__main__":
    ...
    dataset = load_dataset('allenai/common_gen', split='test')
    dataset = dataset.shuffle(seed=42).select(range(args.num_samples))

    batched_prompts = get_prompts("common_gen", dataset, args)
    ...
    outputs_path = Path('data') / f"outputs_common_gen_eval_{mode}_{args.prompt_mode}.json"
    with open(outputs_path, "w") as f:
        json.dump(all_outputs, f, indent=4)

    semantic_diversity = calculate_semantic_diversity(outputs_path)
    with open('eval/common_sense/results/common_gen_results.txt', 'a') as f:
